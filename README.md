...
- **Automated build pipeline** that compiles native Impeller + Rive artifacts on macOS/Linux/Windows, exercises managed tests, and packages NuGet outputs.
...
> Native binaries are produced per RID using `build/native/build_impeller.py` (Impeller) and `build/rive/build_rive.py` (Rive runtime/FFI).
...
# Build native Impeller + Rive binaries for your platform(s).
...
Skip the `--platform` lines for operating systems you do not target. Wrapper scripts (`build/native/build_impeller.sh|ps1`, `build/rive/build_rive.sh|ps1`, `build/managed/package.sh|ps1`) honor the same arguments via environment variables.
...
Both Impeller and Rive native components must be staged before running managed projects or packing NuGet packages.
...
# Rive runtime / FFI bridge
python3 build/rive/build_rive.py --platform macos --arch arm64 --configuration Release
python3 build/rive/build_rive.py --platform linux --arch x64 --configuration Release
python build/rive/build_rive.py --platform windows --arch x64 --configuration Release
```

Each script:
- Initializes required submodules (`extern/flutter`, `extern/rive`) if needed.
- Invokes GN/Ninja (Impeller) or CMake/Ninja (Rive).
- Copies binaries to `artifacts/native/<rid>/native` or `artifacts/rive/<rid>/native` and stages them under `src/*/bin/<Config>/net8.0/runtimes/<rid>/native`.
- Emits `manifest.json` snapshots describing copied files.

Prerequisites per OS are documented in `docs/rive-build-prereqs.md`. Typical dependencies include Ninja, CMake, Python 3, and platform-specific toolchains (Xcode CLI, clang, MSVC).
...
The script:
- Verifies native artifacts exist (Impeller + Rive) unless `--skip-native-check` is supplied.
...
| MotionMark / MotionMarkOriginal | `samples/` | Renders MotionMark-inspired scenes; useful for perf validation once Rive bindings are in place. |
...
| --- | --- | --- |
| Build & Package | `.github/workflows/build.yml` | On pushes/PRs, builds native Impeller + Rive matrices, stages assets, runs managed tests, and packages NuGets (uploads manifest + packages for non-PR events). |
...
Artifacts:
- `native-*` / `native-rive-*` – per-OS native binaries.
- `nuget-packages` / `nuget-manifest` – managed packages + manifest from CI.
- `release-nuget-packages` + `native-impeller.tar.gz` / `native-rive.tar.gz` – release-ready bundles attached to GitHub releases.

---

## Roadmap & References

- `docs/rive-integration-plan.md` – detailed checklist for Rive integration and automation (mirrors progress documented in `docs/rive-status.md`).
...
- **CMake complaints (Rive):** Install platform-specific prerequisites from `docs/rive-build-prereqs.md` (e.g., `brew install cmake ninja` or `sudo apt-get install cmake ninja-build`).
...
- File issues or questions in this repository (use the Rive integration template for plan tracking).
